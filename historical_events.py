...
then uses Claude to convert those real facts into crossword word/clue
...
From these events, select up to {n} that would make good crossword entries. IMPORTANT: select {n} DIFFERENT events -- do not extract more than one answer word from the same event. Each entry must be about a distinct underlying event, not just a distinct word.

For each event you select, extract ONE proper noun or key term as the answer word, and write a "clue" based ONLY on the fact given above for that event, mentioning "this day in {{year}}" using that event's actual year. Do not add any detail not present in the text above.

Return ONLY a JSON array (no markdown fences, no commentary) of objects with "word", "clue". Each "word" must be a single uppercase word, letters only, 4-12 letters, distinct from the others."""
...
def _build_crossword_entries_from_events(events: list[dict], month: int, day: int, n: int = 9) -> list[dict]:
    ...
    # Safety net beyond the prompt instruction: even if Claude ignores the
    # "one answer per event" instruction, catch and drop any entry whose
    # clue overlaps too much with one already kept.
    return dedupe_overlapping_clues(cleaned)


def get_events_for_date(date: datetime.date) -> list[dict]:
    """Returns a list of {"word": ..., "clue": ...} dicts for the given
    date. Tries, in order:
